...
import notificationSystem from './components/NotificationSystem.js?v=155';
...
class App {
  ...
  async handleRoute(route) {
    ...
    switch (page) {
      ...
      case 'home': {
        const { Home } = await import('./pages/Home.js?v=155');
      ...
      case 'collaborators': {
        const { CollaboratorPanel } = await import('./pages/CollaboratorPanel.js?v=155');
        pageInstance = new CollaboratorPanel(this.content, this.currentUser, route.sub);
        break;
      }

      case 'command-center': {
        const { CommandCenter } = await import('./pages/CommandCenter.js?v=155');
        pageInstance = new CommandCenter(this.content, this.currentUser, route.sub);
        break;
      }

      case 'biz-dashboard': {
        const { BusinessDashboard } = await import('./pages/BusinessDashboard.js?v=155');
